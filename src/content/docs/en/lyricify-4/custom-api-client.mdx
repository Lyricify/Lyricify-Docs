...
2. Click `Configure Custom API Client` in welcome page's left bottom corner.  
   <img src={DemoPic05EnUs.src} width="360" />
3. Enter `Client ID` and `Client Secret` you previously obtained.
4. Continue the login process and complete authorization.
...
1. In Lyricify, open the `Configure Custom API Client` dialog by following the steps above.
